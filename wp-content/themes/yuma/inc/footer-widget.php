...
class Yuma_Footer_Widgets{
	...
	function init() {
		...
		if ( $this->active_widgets > 0 ) {
			...
			add_action( $this->theme_prefix . '_footer_start_action', array( $this, 'add_footer_widgets' ), 30 );
		}
	...
	function footer_widgets_init() {
		...
		for ( $i = 1; $i <= $this->max_widgets; $i++ ) {
			register_sidebar( array(
				'name'          => sprintf( __( 'Footer Widget %d', 'yuma' ), $i ),
	...
	function add_footer_widgets() {
		...
		$args = array(
		'container' => 'div',
		'before'    => '<div class="'. $this->custom_footer_widget_class( 'footer-widgets-area' ) .'">',
		'after'     => '</div>',
		);
...
// Initialize.
$yuma_footer_widgets = new Yuma_Footer_Widgets();

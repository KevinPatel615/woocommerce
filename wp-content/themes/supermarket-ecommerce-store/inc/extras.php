 ...
 * @return array
 */
function supermarketecommercestore_body_classes( $classes ) {
...
add_filter( 'body_class', 'supermarketecommercestore_body_classes' );
...
if (!function_exists('supermarketecommercestore_str_replace_assoc')) {

    /**
     * supermarketecommercestore_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function supermarketecommercestore_str_replace_assoc(array $replace, $subject) {
 ...
 */
function supermarketecommercestore_add_to_cart_fragment( $fragments ) {
	...
	<a class="nav-link cart-icon-wrap header-cart" href="javascript:void(0);" role="button" data-bs-toggle="collapse" data-bs-target="#Cart">
		...
		?>	
			<span><?php echo esc_html__('0','supermarket-ecommerce-store'); ?></span>
		<?php
		}
		?>
	</a>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'supermarketecommercestore_add_to_cart_fragment' );
...
if ( ! function_exists( 'supermarketecommercestore_breadcrumb_title' ) ) {
	function supermarketecommercestore_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'supermarket-ecommerce-store' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'supermarket-ecommerce-store' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'supermarket-ecommerce-store' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'supermarket-ecommerce-store' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'supermarket-ecommerce-store' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'supermarket-ecommerce-store' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'supermarket-ecommerce-store' ), (get_the_author( '', false ) ));		
...
function supermarketecommercestore_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','supermarket-ecommerce-store'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','supermarket-ecommerce-store'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','supermarket-ecommerce-store'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
	$before		= '<li class="active">'; // Tag before the current Breadcrumb
	$after 		= '</li>'; // Tag after the current Breadcrumb
	$seprator	= get_theme_mod('seprator','<span style="color: #404040;margin: 0 10px;">/</span>');
	global $post;
	$homeLink = home_url();
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','supermarket-ecommerce-store').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','supermarket-ecommerce-store').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','supermarket-ecommerce-store').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','supermarket-ecommerce-store').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','supermarket-ecommerce-store'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','supermarket-ecommerce-store') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_supermarketecommercestore_dismissed_notice_handler', 'supermarketecommercestore_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function supermarketecommercestore_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
add_action( 'wp_ajax_install_act_plugin', 'supermarketecommercestore_admin_install_plugin' );

function supermarketecommercestore_admin_install_plugin() {
    /**
     * Install Plugin.
     */
